...
// 读取配置档摘要用于二次核对（名称、模型、API URL 主机名）；不输出密钥。
...
// 若删除的是当前配置档，说明删除不会改动 Claude Code 已生效的配置。
function noteIfCurrent(name) {
  if (state.readCurrentProfile() === name) {
    console.log('注意：这是当前由 clis 应用的配置档。');
    console.log('删除它不会修改 Claude Code 已写入的 ~/.claude/settings.json，仅移除该配置档本身。');
    return true;
  }
  return false;
}
...
function performDelete(name) {
  ...
  const cleared = state.clearCurrentIfMatches(name);
